  ...
  return (
    <article className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-card">
      <div className="flex items-start justify-between">
        <span className={`grid size-12 place-items-center rounded-full text-xl ${iconClass}`}>
          {icon}
        </span>
      ...
      <span
        className={`mt-3 inline-flex items-center gap-2 rounded-full px-3 py-1 text-[0.68rem] font-bold ${badgeClass}`}
      >
  ...
  return (
    <section aria-labelledby="overview-title">
      <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          <p className="text-xs font-bold tracking-[0.2em] text-brand-700 uppercase">
            Panel operativo
          </p>
          <h1
            id="overview-title"
            className="mt-1 text-3xl font-extrabold tracking-tight text-slate-950"
          >
            Resumen
          </h1>
          <p className="mt-1 text-sm text-ink-muted">
            Visión general de las actividades de inspección, evaluaciones y riesgo sanitario.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-4">
          <p className="hidden text-right text-xs leading-5 text-brand-900 sm:block">
            Un entorno más seguro
            <br />
            para una mejor salud
          </p>
          <span className="hidden text-3xl text-emerald-500 sm:block" aria-hidden="true">
            ❧
          </span>
        </div>
      </div>
      ...
      <div className="mt-5 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard
          icon="▣"
          iconClass="bg-emerald-100 text-brand-700"
          title="Casos activos"
          value="128"
          trend="+12%"
          description="Inspecciones y casos en seguimiento."
          badge="En seguimiento"
          badgeClass="bg-emerald-50 text-brand-700"
        />
        <KpiCard
          icon="▤"
          ...
          description="Asignaciones por completar."
          badge="Requiere atención"
          badgeClass="bg-amber-50 text-amber-700"
        />
        <KpiCard
          icon="!"
          iconClass="bg-red-100 font-extrabold text-red-500"
          title="Alertas críticas"
          value="8"
          trend="-20%"
          description="Hallazgos de alto riesgo sanitario."
        ...
        <KpiCard
          ...
          value="72%"
          trend="+6%"
          description="Cumplimiento promedio general."
          badge="Tendencia positiva"
          badgeClass="bg-emerald-50 text-brand-700"
        />
      </div>
        ...
        <article className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-card xl:col-span-6">
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-sm font-extrabold text-ink-strong">
              Actividad de inspecciones y evaluaciones
            </h2>
            <span className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-ink-body">
              Últimos 12 meses
            </span>
          </div>
          <div className="relative mt-5 h-52 overflow-hidden rounded-xl bg-[linear-gradient(to_right,rgba(148,163,184,.15)_1px,transparent_1px),linear-gradient(to_bottom,rgba(148,163,184,.15)_1px,transparent_1px)] bg-[size:48px_34px]">
            <div
              className="absolute inset-x-5 bottom-7 flex h-36 items-end justify-between gap-2"
              aria-label="Gráfico ilustrativo de actividad"
            >
              {[42, 55, 38, 64, 48, 68, 76, 88, 52, 61, 72, 82].map((height, index) => (
                <div key={index} className="flex h-full flex-1 items-end">
                  <span
                    className="w-full rounded-t bg-gradient-to-t from-brand-700 to-emerald-400"
                    style={{ height: `${height}%` }}
                  />
                </div>
              ))}
            </div>
            <p className="absolute inset-x-0 bottom-1 text-center text-[0.62rem] text-ink-muted">
              Actividad mensual consolidada
            </p>
          </div>
        </article>
            ...
            <div className="grid size-36 shrink-0 place-items-center rounded-full bg-[conic-gradient(#11754c_0_55%,#65c77b_55%_83%,#f5b422_83%_95%,#e83a4f_95%)]">
              <div className="grid size-24 place-items-center rounded-full bg-white text-center">
                <div>
                  ...
                  <p className="text-[0.6rem] text-ink-muted">Cumplimiento</p>
                </div>
              </div>
            </div>
            <div className="space-y-2 text-[0.68rem]">
              <p>
                <span className="mr-2 inline-block size-2.5 rounded-full bg-brand-700" />
                Bajo 55%
              </p>
              <p>
                <span className="mr-2 inline-block size-2.5 rounded-full bg-green-400" />
                Medio 28%
              </p>
              <p>
                <span className="mr-2 inline-block size-2.5 rounded-full bg-amber-400" />
                Alto 12%
              </p>
              <p>
        ...
        <article className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-card xl:col-span-3">
          ...
          <div className="mt-4 flex h-44 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-50 to-white">
            <div className="text-center text-brand-600">
              <span className="block text-7xl opacity-35">⌖</span>
              <p className="mt-2 text-[0.65rem] text-ink-muted">Mapa de cobertura territorial</p>
            </div>
          </div>
        </article>
      </div>
      ...
      <div className="mt-4 grid gap-4 pb-4 xl:grid-cols-12">
        <article className="overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-card xl:col-span-7">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[720px] text-left text-xs">
              <thead className="border-y border-slate-100 bg-slate-50 text-[0.62rem] tracking-wide text-ink-muted">
                <tr>
                  {[
                    'FECHA',
                    'CÓDIGO',
                    ...
                    <th key={heading} className="px-3 py-2.5 first:pl-5">
                      {heading}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-slate-100 text-ink-body">
                {evaluations.map(([date, code, establishment, type, status, risk]) => (
                  <tr key={code} className="hover:bg-slate-50">
                    <td className="py-3 pr-3 pl-5">{date}</td>
                    <td className="px-3 py-3 font-bold">{code}</td>
                    <td className="px-3 py-3 text-ink-strong">{establishment}</td>
                    <td className="px-3 py-3">{type}</td>
                    <td className="px-3 py-3">
                      <span
                        className={`rounded-full px-2.5 py-1 font-bold ${status === 'Completada' ? 'bg-emerald-50 text-emerald-700' : status === 'En proceso' ? 'bg-amber-50 text-amber-700' : 'bg-red-50 text-red-600'}`}
                      >
                        {status}
                      </span>
                    </td>
                    <td
                      className={`px-3 py-3 font-bold ${risk === 'Bajo' ? 'text-emerald-600' : risk === 'Medio' ? 'text-amber-600' : 'text-red-600'}`}
                    >
                      {risk}
                    </td>
        ...
        <article className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-card xl:col-span-2">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-extrabold">Hallazgos</h2>
            <a
              href="/modulo.html?module=hallazgos"
              className="text-[0.65rem] font-bold text-brand-700"
            >
              Ver todos
            </a>
          </div>
          <ul className="mt-3 divide-y divide-slate-100 text-xs">
            {findings.map(([icon, label, count, color]) => (
              <li key={String(label)} className="flex items-center gap-2 py-2.5">
                <span className={`grid size-7 place-items-center rounded-lg ${color}`}>{icon}</span>
        ...
        <article className="rounded-2xl border border-slate-200/80 bg-white p-5 shadow-card xl:col-span-3">
          <div className="flex items-center justify-between">
            <h2 className="text-sm font-extrabold">Próximas acciones</h2>
            <button type="button" className="text-[0.65rem] font-bold text-brand-700">
              Ver todas
            </button>
          </div>
          <div className="mt-3 space-y-2 text-xs">
            {actions.map(([day, month, title, subtitle, border]) => (
              <div key={String(title)} className="flex gap-3 rounded-xl bg-slate-50 p-2.5">
                <div className={`w-10 shrink-0 border-r-2 text-center ${border}`}>
                  <strong className="block text-base">{day}</strong>
                  <span className="text-[0.55rem] text-ink-muted">{month}</span>
                </div>
                <div>
