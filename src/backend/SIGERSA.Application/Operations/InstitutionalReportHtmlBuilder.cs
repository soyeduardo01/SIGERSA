...
public static class InstitutionalReportHtmlBuilder
{
    ...
    public static string Build(ReportGenerationData data, bool official, InstitutionalReportAssets assets)
    {
        ...
        return $$"""
            ...
            {{PageStart(assets, data, "Hallazgos y recomendaciones", "Detalle técnico de la inspección")}}{{Title("3", "Hallazgos y no conformidades")}}<div class="summary-strip">{{SummaryPill(data.Findings.Count, "Hallazgos totales")}}{{SummaryPill(openFindings, "Pendientes de cierre")}}{{SummaryPill(priorityFindings, "Prioridad alta")}}</div>{{findings}}<div class="section">{{Title("4", "Recomendaciones")}}<ol class="recs">{{string.Join(string.Empty, recommendations.Select(item => $"<li>{E(item)}</li>"))}}</ol></div>{{Footer(data, 3)}}</section>
            {{PageStart(assets, data, "Anexo de evidencias", "Índice documental asociado")}}<div class="evidence-identity">{{Logos(assets)}}<span><b style="font-size:9px">Cadena documental verificable</b><small style="display:block;color:#5c6b66;font-size:7px;margin-top:3px">Registro, asociación y conservación de evidencias</small></span></div><div class="process-flow">{{ProcessStep("01", "Captura", "Archivo original")}}<span class="flow-arrow">→</span>{{ProcessStep("02", "Asociación", "Evaluación y hallazgo")}}<span class="flow-arrow">→</span>{{ProcessStep("03", "Custodia", "Expediente trazable")}}</div><p style="font-size:9px;color:#5c6b66">Los archivos originales se incorporan a continuación y conservan su formato.</p>{{evidence}}{{Footer(data, 4)}}</section>
