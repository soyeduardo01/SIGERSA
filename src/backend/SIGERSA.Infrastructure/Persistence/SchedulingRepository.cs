    ...
    : DapperRepositoryBase(connectionFactory), ISchedulingRepository
{
    public async Task<SchedulesPage> SearchAsync(ScheduleSearch query, CancellationToken cancellationToken = default)
    {
        const string sql = """
            SELECT p.id AS Id, p.caso_id AS CaseId, c.numero AS CaseNumber,
             ...
             GROUP BY p.id, c.numero, e.razon_social, est.nombre
            ...
            SELECT COUNT(*) FROM "SIGERSA"."PROGRAMACION" AS p
             ...
             WHERE (@Status IS NULL OR p.estado = @Status)
               AND (@Search IS NULL OR lower(c.numero) LIKE '%' || @Search || '%'
                    ...
                    OR lower(est.nombre) LIKE '%' || @Search || '%');
            """;
        var parameters = new { query.Search, query.Status, Offset = (query.Page - 1) * query.PageSize, query.PageSize };
    ...
    {
        const string sql = """
            SELECT c.id AS Id, c.numero || ' · ' || e.razon_social || ' · ' || est.nombre AS Name
    ...
    public async Task<Guid> CreateAsync(ScheduleDraft draft, Guid actorId, CancellationToken cancellationToken = default)
    {
        ...
        await using (var transaction = await connection.BeginTransactionAsync(cancellationToken))
        {
            ...
            await ReplaceAssignmentsAsync(connection, transaction, id, draft.EvaluatorIds, actorId, cancellationToken);
    ...
    private static async Task ReplaceAssignmentsAsync(System.Data.Common.DbConnection connection, System.Data.Common.DbTransaction transaction,
        Guid scheduleId, Guid[] evaluatorIds, Guid actorId, CancellationToken cancellationToken)
    {
        await connection.ExecuteAsync(new CommandDefinition(Sql("""
            ...
            INSERT INTO "SIGERSA"."ASIGNACION"
                (id, programacion_id, evaluador_id, es_principal, estado, asignado_por, creado_por)
            SELECT gen_random_uuid(), @ScheduleId, value, ordinality = 1, 'ACTIVA', @ActorId, @ActorId
            ...
            """), new { ScheduleId = scheduleId, EvaluatorIds = evaluatorIds, ActorId = actorId }, transaction, cancellationToken: cancellationToken));
    }
    ...
    private sealed class ScheduleRow
    {
        ...
        public ScheduleRecord ToDomain() => new(Id, CaseId, CaseNumber, CompanyName, EstablishmentName,
